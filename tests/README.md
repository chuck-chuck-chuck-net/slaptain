...
| `E2E_ACCESSLOG_MIGRATION` | *(unset)* | Set to `1` to run the ADR-019 R8 legacy-accesslog migration scenario (`accesslog_migration_test.go`, label `accesslog-migration`). It manufactures the pre-ADR-019 layout in `cn=config` on every RW pod — drops the per-database logs, inserts one `cn=accesslog` at `/accesslog` **below** the data databases so its deletion renumbers their `olcDatabase={N}` DNs, and repoints both overlays at it — then asserts the operator converges to the per-database end state, that the convergence is idempotent, and that delta-syncrepl survives it. Opt-in because it rewrites `cn=config` by hand and discards the cluster's journals. |
