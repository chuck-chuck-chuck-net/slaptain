...
var _ = Describe("resilience", Label("resilience"), Ordered, func() {
	...
	// Warm-restart test: every pod's data survives across the restart via its
	// PVC. Pair with dataloss_recovery_test.go, which covers the cold case
	// (single pod loses its PVCs entirely and recovers via syncrepl).
